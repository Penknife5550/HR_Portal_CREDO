...
### E6 — Fristen, Aufbewahrung & Löschung  ·  ~2–3 Tage  ·  ✅ UMGESETZT (2026-06-08)
- [x] `src/lib/bem-fristen.ts`: `berechneBemFristTemplates` (EINLADUNG/EINWILLIGUNG/ERSTGESPRAECH/FOLGEGESPRAECH/EVALUATION/AUFBEWAHRUNGSENDE) + `berechneSeverity` (INFO/WARNING/URGENT/OVERDUE) + `syncBemFristen`. Eingehängt in Anlage/Transition/Einladung/Einwilligung(digital+Papier).
- [x] `POST /api/cron/bem-fristen` (CRON_SECRET + timingSafeCompare): synct Fristen, sendet bei Eskalation **eine interne Erinnerungs-Mail pro Fall** an die freigegebenen Beauftragten (SMTP-direkt, CREDO-CI, **keine Gesundheitsdaten** — nur Fall-Nr + Bezeichnung + Stichtag), Versandnachweis in `BemKommunikation` + Audit (`BEM_FRIST_ERINNERUNG`). `letzteSeverity` verhindert Mehrfach-Mails.
- [x] `POST /api/cron/bem-aufbewahrung` (CRON_SECRET): `aufbewahrungBis <= heute` → Crypto-Shredding (Gespräche/Maßnahmen/Einwilligungen/Dokumente + Dateien gelöscht, Fall→`GELOESCHT`+`geloeschtAm`; AuditLog + BemKommunikation bleiben als Nachweis). DB-Transaktion zuerst, Dateien danach (best-effort, pfad-sicher).
- [x] UI: „Offene Fristen"-Karte in der Fall-Übersicht (mit Severity-Farbe). Tests `bem-fristen.test.ts` (8). Build/Lint grün.
- **DoD:** ✅ Fristen erscheinen mit Severity; Erinnerungen automatisch (SMTP-direkt, kein Leak); Lösch-Cron entfernt abgelaufene Inhalte + behält Audit; Aufbewahrungsdatum aus `beendetAm + bemAufbewahrungJahre` (in `bem-transitions.ts`). ⏳ **Server: beide Crons in n8n einplanen** (täglich, POST + Bearer `CRON_SECRET`) — siehe `docs/BEM_DEPLOY.md`.

### E6 (Original-Planungstext)  ·  ~2–3 Tage
...
- **G4 — Mandanten-Bindung bei Freigaben · ✅ UMGESETZT (2026-06-09)** `POST /api/bem/[id]/zugriffe` prüft jetzt, dass die freizugebende Person einen legitimen Bezug zum Mandanten des Falls hat: globale Rolle, interne:r BEM-Beauftragte:r (Flag), externe:r `BEM_BEAUFTRAGTER` (fallbasiert) oder Org-Zuweisung zum Fall-Mandanten — sonst 422. Verhindert versehentliche mandantenübergreifende Freigaben.

**Stand:** BEM-Modul E0–E9 vollständig umgesetzt; Phase-1- und Phase-2-Pakete (außer E8 Fehlzeiten-Automatik) abgeschlossen, jede Phase adversariell reviewt und gehärtet. Offen nur noch **E8** (automatische Auslösung aus Fehlzeiten, LOGA/n8n).
