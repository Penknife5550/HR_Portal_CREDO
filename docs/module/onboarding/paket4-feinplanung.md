...
| U-22 | Wortlaut Z3: `dokument-abgelaufen` sagt „… dann fordern Sie diese im Vorgang über ‚Unterlagen nachfordern‘ an; sobald Sie sie als `{{dokument_typ}}` mit ihrem Ablaufdatum annehmen, endet die Warnung“. `dokument-ablauf-warnung` nennt zusätzlich die Annahme als „Unbefristet“ (Z1). Beide Sätze stehen nur, wo das Portal die Nachforderung anbietet (U-37). | Eine Fiktionsbescheinigung beendet die Warnung nur, wenn HR sie als dieselbe Art mit Datum annimmt — der Lauf mahnt Aufenthaltstitel UND Arbeitserlaubnis, je Art getrennt. Eine Niederlassungserlaubnis beendet die Erinnerung nur über das Kennzeichen „unbefristet“. |
| U-23 | Eine Erinnerung, die die SMTP-Bremse zurückhält, legt keinen Link an. Der Lauf-Wächter meldet am Folgetag nur eine ausgebliebene **Vorab**-Erinnerung als „nicht versendet“. Eine zurückgehaltene **Fristtag**-Erinnerung meldet er nicht, und nachgeholt wird sie auch nicht (ein verpasster Fristtag entfällt); sichtbar ist sie nur als `nichtZugestellt`/`GEBREMST` im Bericht des Laufs. | Gewollt: Die Erinnerung ist tatsächlich ausgeblieben. Den Wächter um den Fristtag zu erweitern hieße, einen Alarm zu melden, den niemand mehr beheben kann — der Tag ist vorbei. |
...
**Nachbesserung aus der Abschlussdurchsicht (25.09.2026)**

Belegte Befunde der Abschlussdurchsicht, behoben in der Arbeitskopie nach `2ef8199`. Wo ein Punkt einem früheren widerspricht, gilt er.

| # | Umsetzung | Grund |
|---|---|---|
| U-32 | **Grenze „eigene Entwürfe“ unter der Sperre** (präzisiert U-17): `entwertetAm` eines Adresswechsels („Link erneut senden“ an eine neue Adresse) und `hochgeladenAm` eines Entwurfs nimmt jeder Weg erst NACH der Zeilensperre der Nachforderung (`zeitpunktUnterSperre` in `unterlagen-dienst.ts`, nie früher als `jetzt`), nicht mit dem Anfang der Anfrage. Die Regel selbst steht einmal rein in `unterlagen.ts` (`entwuerfeAb`, `eigenerEntwurf`) — für die Upload-Seite und den Merker `entwurf_vorhanden` des Laufs. | Mit dem Anfang der Anfrage konnte ein Upload über den alten Link, der die Sperre VOR HR bekam, hinter der Entwertung liegen und als eigener Entwurf der neuen Adresse zählen. Jetzt bestimmt die Reihenfolge der Sperre die der Zeitstempel. |
| U-33 | **Mails ohne Ergebnis klärt der Lauf vor der Planung** (präzisiert N2 und Abschnitt 9): Steht ein Link seit über einer Stunde auf AUSSTEHEND, sucht der Lauf im `EmailLog` ein SENT desselben Ereignisses (`personenEventFuerAnlass`) an dieselbe Adresse, nach dem Anlegen des Links und vor dem nächsten Link der Nachforderung (höchstens eine Stunde). Gefunden: SENT mit Zeit und Message-ID des Protokolls, keine zweite Mail; bei „frühere Links sperren“ am jüngsten Link sperrt er im selben Commit. Sonst FAILED „kein Ergebnis“ — auch bei Erinnerungen. Bericht: Schritt `ERGEBNIS` (im Probelauf `GEPLANT`, nur gelesen). | Nach N2 las HR „versendet, bitte nicht erneut senden“, und der nächste Lauf hielt genau diesen Link für einen Absturz, markierte die zugestellte Mail rot und schickte sie noch einmal. Erinnerungs-Links ohne Ergebnis standen für immer auf „wird gesendet“. Restrisiko: Fehlt auch der Protokolleintrag (Datenbank ganz weg), gilt die Mail als nicht zugestellt und wird nachgeholt; den Merker einer Erinnerung trägt der Abgleich nicht nach. |
| U-34 | **Lauf-Wächter nach einer Friständerung** (präzisiert Abschnitt 9 und EP-13): Kein Alarm, wenn HR gestern oder heute eine Mail mit fristsetzendem Anlass angelegt hat (Anfordern, Ergänzen, Frist ändern, Zurückweisen — gleich mit welchem Ergebnis). Ab dem Tag danach gilt die Regel wieder. | Rückt HR die Frist ins Vorab-Fenster und scheitert die Mail dazu, meldete der Wächter „Lauf erreicht das Portal vermutlich nicht“, obwohl der Lauf gestern die neue Frist gar nicht kennen konnte. Ein Lauf, der wirklich ausfällt, bleibt so höchstens einen Tag länger unbemerkt. |
| U-35 | **„(neu)“ beim Nachholen einer Ergänzung**: Bezug ist der Link von HR, mit dem die Kette der Versuche begann (jüngster Link desselben Anlasses mit `nachholVersuche = 0`), nicht der jüngste. | Ab dem zweiten Nachholversuch ist der jüngste Link der des Laufs vom Vortag; alle Positionen wären älter, und die Mail markierte nichts mehr als neu. |
| U-36 | **Verknüpfen bei Annehmen und Rücknahme** wiederholt bei ENOENT wie `entwurfSpeichern` (höchstens drei Versuche); fehlt die Quelle selbst, folgt `DATEI_FEHLT` (409). | Der öffentliche Weg „Entfernen“ räumt ohne Prozesssperre den leer gewordenen Ordner der Nachforderung ab. Fiel das zwischen `mkdir` und `link`, antwortete die Rücknahme mit 500. |
| U-37 | **Z3 nur, wo das Portal die Nachforderung anbietet** (ergänzt Z3, korrigiert U-22): Die Handlungsanweisung beider Vorlagen steht in `{{#nachforderung_moeglich}}…{{/nachforderung_moeglich}}`; bei gesperrter Nachforderung nennt `{{#nachforderung_gesperrt}} {{nachforderung_hinweis}}{{/nachforderung_gesperrt}}` Grund und Weg (`ONBOARDING_NACHFORDERUNG_GESPERRT_MAIL`, aus derselben Regel wie die Karte: `onboardingNachforderungGesperrt` — EXPIRED oder Fragebogen offen). Welche Mails `dokument-ablauf` verschickt, ändert sich nicht (bewusst ohne Statusfilter). `dokument-abgelaufen` setzt die Art als `{{dokument_typ}}`. | Der Lauf mahnt auch EXPIRED-Vorgänge und Vorgänge mit offenem Fragebogen; dort lehnt der Dienst die Nachforderung mit 409 ab. Und „als Aufenthaltstitel annehmen“ beendete die Mahnung zur Arbeitserlaubnis nie. Beide Vorlagen bekommen damit einen neuen Standard: „Text auf Standard zurücksetzen“ nach dem Deploy (steht schon im Ablaufplan), die Prüfsummen dort sind neu berechnet. |
| U-38 | Das Katalogbeispiel der HR-Mails trägt `portalLink` mit `?tab=dokumente` (wie U-25); ein Test hält es gegen `portalPfad` des Bausteins. | Testversand, Vorschau und Webhook-Abnehmer lernen die URL-Form aus dem Katalog. |
| U-39 | **„Frist ändern…“ nennt die anfordernde HR-Kraft**: `dialogErinnerungsSatz(frist, heute, "ANFORDERNDE")` — „… erhält die anfordernde HR-Kraft eine E-Mail“, wie beim Ergänzen. „Sie“ nur beim Anfordern selbst. | Die Mail „Frist verstrichen“ geht an `angefordertVon` (8.1), nicht an die Person, die gerade die Frist ändert. |
| U-40 | **Warnbalken und Arbeitserlaubnis** (präzisiert 10.2 und U-26): Beim Aufenthaltstitel kreuzt „Verlängerten Nachweis anfordern…“ die Arbeitserlaubnis mit an (`verlaengerungVorauswahl`), auch wenn ihr eigener Nachweis nicht abläuft. HR nimmt das Häkchen heraus, wenn sie nicht gebraucht wird. | Die Pflichtregel verlangt beide Arten, und meist steht die Erlaubnis auf dem Titel selbst (N4). Handbuch angepasst („genau dieser Nachweis“ stimmte nicht). |
| U-41 | **Kasten „Offene Nachweise“**: Der Satz zählt auch Arten, die in der LAUFENDEN Nachforderung als entfallen vermerkt sind (`alsEntfallenVermerkt`); `KASTEN_ALLE_ENTFALLEN` sagt „Laut Nachforderung …“ statt „Laut einer früheren Nachforderung …“. | Kein Knopf kreuzt eine in der laufenden entfallene Art an; „genau diese Nachweise“ stimmte für sie nicht. |
| U-42 | **„Annahme zurücknehmen…“ bei EXPIRED** (ergänzt EP-3/Z2): Die Rückfrage nennt VOR der Aktion die Folge aus Z2 (`nachforderung.dialog.ruecknahmeFolge` = `MELDUNGEN.RUECKNAHME_VORGANG_EINGESTELLT`): Wird die Unterlage nicht vor dem nächsten Lauf erneut angenommen, zieht er die Nachforderung zurück, die Datei wird verworfen; zurückweisen geht nicht mehr. | EP-3 erlaubt die Rücknahme ausdrücklich; die Folge stand aber erst nach der Aktion in der Karte. |
| U-43 | **Eine Regel, ein Ort** (Aufräumen ohne Verhaltensänderung): `WARTET_AUF_PERSON_STATUS`/`wartetAufPerson` auch in Mails und Prisma-Bedingungen; `fehlerKennung` in `src/lib/fehler-kennung.ts` (auch für `nachDerAntwort`); `NachholAnlass` aus `NACHHOLBARE_ANLAESSE` abgeleitet, `UnterlagenAufforderungAnlass` daraus; `mailStatus`/`entwertetGrund` an ihren Schreibstellen per `satisfies` an `MailStatus`/`EntwertungGrund` gebunden; `DateitypErgebnis` ohne `endung` (sie steht nur in `ENDUNG_FUER_DATEITYP`); die Übersicht ohne `mailVerlaufText`, `linkAbgelaufen` und `empfaengerAbweichend`; `dialogHeute` (Tag des Servers, nie früher als im Browser) in allen drei Frist-Dialogen und `adressFormatFehler` in beiden Adressdialogen (`aktionen.ts`, „Link erneut senden“ prüft jetzt das Format); die HR-Routen nennt nur noch der Server (`apiBasis`, keine Tabelle `API_BASIS` im Client). | Kopien liefen auseinander, sobald eine geändert wird — zwei davon schon (Frist-Dialoge über Mitternacht, Adressprüfung). |
