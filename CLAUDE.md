...
- **Versand:** `src/lib/dokumentenpaket.ts` (modulneutral) hinter drei Routen — `GET /api/dokumentenpaket` (Zusammenstellung fuer den Dialog), `POST .../pruefen` (Vorpruefung), `POST .../versenden`. Oberflaeche: `dokumentenpaket-section.tsx` + `dokumentenpaket-dialog.tsx`. Phase 1 verdrahtet **nur ONBOARDING**; die drei anderen Module brauchen je eine eigene Mailvorlage (Phase 2). Die abgeloeste `starterpaket.ts` und `POST /api/onboarding/[id]/starterpaket` gibt es nicht mehr.
