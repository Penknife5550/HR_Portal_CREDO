...
import { ElternzeitAntragForm } from "./elternzeit-antrag-form";

export interface ElternzeitAntragData {
  elternzeitId: string;
  displayId: string;
  employeeName: string;
  ...
  kindNummer: number;
  ferienBegruendungPflicht: boolean;
  schulferien: {
    bezeichnung: string;
    von: string;
    bis: string;
...
export default function ElternzeitAntragPage() {
  const params = useParams();
  const token = params.token as string;

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<ElternzeitAntragData | null>(null);
  ...
  return <ElternzeitAntragForm token={token} initialData={data!} />;
}
