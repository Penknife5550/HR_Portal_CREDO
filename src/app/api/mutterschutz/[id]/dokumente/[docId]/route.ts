...
export async function GET(
  ...
  try {
    ...
    const doc = await prisma.mutterschutzDokument.findUnique({
      where: { id: docId },
      include: { mutterschutz: { select: { organizationId: true } } },
    });
    if (!doc || doc.mutterschutzId !== id) {
      return NextResponse.json({ error: "Dokument nicht gefunden" }, { status: 404 });
    }
    if (!(await canAccessProcess(session, doc.mutterschutz.organizationId))) {
      return NextResponse.json({ error: "Dokument nicht gefunden" }, { status: 404 });
    }

    // Path-Traversal-Schutz: Datei muss innerhalb von uploads/mutterschutz/<id>/ liegen
    ...
    try {
      ...
      console.error("[API] mutterschutz/dokumente GET Datei-Lesefehler:", readErr);
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.mutterschutzDokument.delete({ where: { id: docId } });
    ...
    console.error("[API] mutterschutz/dokumente DELETE fehlgeschlagen:", error);
    return NextResponse.json({ error: "Interner Serverfehler" }, { status: 500 });
  }
}
